...
- Первый пост — анонс (`gitTrendIntro.ts`); расписание в анонсе: GitHub **10:40**, «Направление недели» **11:20**
...
- Цель длины: **2500–3500** символов (4–5 предложений в summary и в каждом тренде)
- Максимум Telegram: **4096** (`sendPost` → `sendMessage`)
- Нормализация горизонта: GPT может вернуть `1-3 года` → маппинг в `1-3 years` (`generateWeeklyTrends.ts`)
- При переборе — мягкая подгонка описаний, а не обрезка summary до 200 символов

### Лимиты Telegram по рубрикам

| Рубрика | Доставка | Генерация | API-лимит |
| --- | --- | --- | --- |
| Направление недели | текст | 2500–3500 → fit 4096 | 4096 |
