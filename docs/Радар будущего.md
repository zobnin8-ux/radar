...
| --- | --- | --- | --- |
| **Ср, Сб** 10:20 | 📦 Будущее в коробке | `/box` | `20 10 * * 3,6` |
| **Вс** 11:00 | 🧭 Направление недели | `/trends` | `0 11 * * 0` |
| **Вс** 10:30 | 🔮 GitHub-сигналы | `/github` | `30 10 * * 0` |
...
| **Radar** | **Вс 10:30** локально | Скачивает JSON → посты в канал |

```text
Сб 21:00 МСК  →  GitTrend push JSON
Вс 10:30 МСК  →  Radar fetch → enrich → Telegram
```

> Сдвиг 10:30 вместо 10:00 — буфер от publish tick (`0 */2 * * *`).
...
- Cron **среда** и **суббота** 10:20 — отдельные слоты
...
2. **Среда, суббота 10:20:** «Будущее в коробке»
3. **Суббота 21:00 МСК:** GitTrend пушит JSON
4. **Воскресенье 10:30:** Radar → «GitHub-сигналы»; **11:00** — «Направление недели»
...
| 2026-06-14 | GitTrend: сб 21:00 МСК → Radar вс 10:30; коробка 10:20 |
